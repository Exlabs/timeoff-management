...
module.exports = function(sequelize, DataTypes) {
  ...
  var User = sequelize.define("User", {
      ...
      people_of_contact_ids : {
        type          : DataTypes.TEXT,
        allowNull     : false,
        defaultValue  : '[]', // Stores the array as a JSON string
        comment       : 'Collection of users who will be notified when users submits a leave request',
        get: function() {
          const rawValue = this.getDataValue('people_of_contact_ids');
          return JSON.parse(rawValue);
        },
        set: function(value) {
          this.setDataValue('people_of_contact_ids', JSON.stringify(value));
        }
      }
  }, {
      indexes : [
...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    promise_people_of_contact : function() {
      return sequelize.models.User.findAll({ id: { $in: this.people_of_contact_ids }});
    },
