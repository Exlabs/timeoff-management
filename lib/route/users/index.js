...
router.get('/edit/:user_id/', function(req, res){
  ...
  Promise
    ...
    .then(async function(company) {
      ...
      const people_of_contact = await model.User.findAll({ where: { id: { $in: employee.people_of_contact_ids }}});
...
router.get('/available-people-of-contact/:user_id', function(req, res) {
  var user_id = req.params['user_id'],
      model = req.app.get('db_model');

  model.User.find({where : {id : user_id}})
    .then((user) => user.people_of_contact_ids)
    .then((user_ids) => {
      var user_ids_to_exclude = user_ids.concat([user_id]);
      return Promise.resolve(model.User.findAll({ where: { id: { $notIn: user_ids_to_exclude } } }));
    })
    .then((users) => {
      return res.render('user/available_people_of_contact', {
        layout: false,
        users
      });
    });
...
router.post('/edit/:user_id/people_of_contact', function(req, res) {
  var user_id = validator.trim(req.params['user_id']),
      employee;
  ...
  Promise.try(
    () => ensure_add_people_of_contact_action({req: req})
  ).then(
    () => ensure_user_id_is_integer({req : req, user_id : user_id})
  ).then(
    () => req.user.get_company_for_user_details({ user_id: user_id })
  ).then(
    (company) => employee = company.users[0]
  ).then(
    () => promise_to_update_people_of_contact({
        user: employee,
        people_of_contact_id: req.body.person_of_contact_id,
    }).then(() => req.session.flash_message('People of contact were updated.'))
  ).then(
    () => res.redirect_with_session('.')
  );
});

var handle_remove_person_of_contact = (args) => {
  var req = args.req;
  var res = args.res;
  var user_id = args.user_id;
  var person_of_contact_id_to_remove = args.person_of_contact_id_to_remove;
  ...
  Promise.try(
    ...
    () => req.user.get_company_for_user_details({ user_id })
  ).then(
    (company) => employee = company.users[0]
  ).then(
    () => promise_to_delete_person_of_contact({
      user: employee,
      person_of_contact_id: person_of_contact_id_to_remove
    }).then(() => req.session.flash_message('Person of contact was removed.'))
  ).then(
    () => res.redirect_with_session('.')
  );
};
...
router.post('/edit/:user_id/', function(req, res) {
  ...
  if (req.body.remove_person_of_contact_id) {
    var person_of_contact_id_to_remove = req.body.remove_person_of_contact_id;

    return handle_remove_person_of_contact({
      req,
      res,
      user_id,
      person_of_contact_id_to_remove
    });
  }
...
function promise_to_update_people_of_contact(args) {
  var user = args.user;
  var new_people_of_contact_ids = args.people_of_contact_id;

  var makeArrUnique = (array) => [...new Set(array)];

  var people_of_contact_ids = makeArrUnique(user.people_of_contact_ids.concat(new_people_of_contact_ids))
    .map((el) => parseInt(el));

  return user.updateAttributes({ people_of_contact_ids });
}

function promise_to_delete_person_of_contact(args) {
  var user = args.user;
  var person_of_contact_id = args.person_of_contact_id;

  var people_of_contact_ids = user.people_of_contact_ids.filter((el) => el != parseInt(person_of_contact_id));
...
function ensure_add_people_of_contact_action(args) {
  var req = args.req;

  if (req.body.do_add_people_of_contact != '1') {
    throw new Error('User ' + req.user.id + ' tried to perform unexpected action');
  }
}
